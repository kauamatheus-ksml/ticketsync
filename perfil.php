...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    if (isset($_SESSION['userid'])) {
        ...
        $table = "clientes";
        // Verifica se é aniversariante
        $sql_aniversariante = "SELECT * FROM aniversariantes WHERE id='$userid'";
        $result_aniversariante = $conn->query($sql_aniversariante);
        if ($result_aniversariante->num_rows > 0) {
...
if (isset($_SESSION['userid'])) {
    ...
    // Verifica se é cliente Paula
    $sql_cliente_paula = "SELECT * FROM clientes_paula WHERE id='$userid'";
    $result_cliente_paula = $conn->query($sql_cliente_paula);
    if ($result_cliente_paula->num_rows > 0) {
        $table = "clientes_paula";
    }
} elseif (isset($_SESSION['adminid'])) {
    $userid = $_SESSION['adminid'];
    $table = "administradores";

...
$sqlIngressosAdquiridos = "
    SELECT vendas.*, 
           ...
           CASE 
               WHEN clientes.id IS NOT NULL THEN clientes.nome
               WHEN clientes_paula.id IS NOT NULL THEN clientes_paula.nome
               WHEN aniversariantes.id IS NOT NULL THEN aniversariantes.nome
           END AS cliente_nome, 
           CASE 
               WHEN clientes.id IS NOT NULL THEN 'cliente'
               WHEN clientes_paula.id IS NOT NULL THEN 'cliente_paula'
               WHEN aniversariantes.id IS NOT NULL THEN 'aniversariante'
           END AS tipo_usuario
    FROM vendas
    ...
    LEFT JOIN clientes_paula ON vendas.usuario_id_cliente_paula = clientes_paula.id
    LEFT JOIN aniversariantes ON vendas.usuario_id_aniversariante = aniversariantes.id
